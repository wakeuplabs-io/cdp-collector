...
import { generateInvitation } from "./utils/invitation";
...
describe("Distributor", function () {
  async function deployDistributorFixture() {
    ...
    const usdc = await hre.viem.deployContract("src/mocks/USDC.sol:USDC", []);
    ...
    await usdc.write.mint([creator.account.address, mintAmount]);
    await usdc.write.mint([addr1.account.address, mintAmount]);
    await usdc.write.mint([addr2.account.address, mintAmount]);
    await usdc.write.mint([addr3.account.address, mintAmount]);
  ...
  async function deployDistributorFixtureWithDonations() {
    ...
    await usdc.write.mint([owner.account.address, mintAmount]);
    await usdc.write.mint([creator.account.address, mintAmount]);
    await usdc.write.mint([donor.account.address, mintAmount]);
    await usdc.write.mint([addr3.account.address, mintAmount]);
    ...
    await usdc.write.approve([distributor.address, donationAmount * 2n], {
      account: donor.account,
    });
  ...
  describe("Deployment", function () {
    it("Should set the correct USDC address", async function () {
      ...
      expect(await distributor.read.usdc()).to.equal(getAddress(usdc.address));
    });
  ...
  describe("Donations", function () {
    it("Should allow donations to active pools", async function () {
      const { distributor, usdc, addr2, singleMemberPoolId, creator } =
      ...
      await usdc.write.approve([distributor.address, donationAmount], {
        account: addr2.account,
      });
      ...
      const balanceAfter = await usdc.read.balanceOf([creator.account.address]);
      expect(balanceAfter - balanceBefore).to.equal(donationAmount);
    });

    it("Should emit DonationMade event", async function () {
      const { distributor, usdc, addr2, singleMemberPoolId, publicClient } =
        await loadFixture(deployDistributorFixtureWithPool);
    ...
    it("Should revert donation to non-existent pool", async function () {
      const { distributor, usdc, addr1 } = await loadFixture(
        deployDistributorFixture
      );
    ...
    it("Should revert donation to inactive pool", async function () {
      const { distributor, usdc, addr2, deactivatedPoolId } = await loadFixture(
        deployDistributorFixtureWithPool
      );
    ...
    it("Should revert donation to pending pool (with invitation slots)", async function () {
      const { distributor, usdc, addr1, addr2 } = await loadFixture(
        deployDistributorFixture
      );
    ...
    it("Should split donations immediately to member balances", async function () {
      const { distributor, usdc, singleMemberPoolId, creator, addr2 } =
        await loadFixture(deployDistributorFixtureWithPool);
  ...
  describe("Invitation Codes", function () {
    ...
    it("Should allow joining with correct invitation code", async function () {
      const { distributor, usdc, creator, addr2 } = await loadFixture(
        deployDistributorFixture
      );
    ...
    it("Should emit MemberJoined event when joining", async function () {
      const { distributor, usdc, creator, addr2, publicClient } =
    ...
    it("Should reject joining slot without invitation code", async function () {
      ...
      await distributor.write.createPool(
        [
          ...
          "Description",
          "https://example.com/image.png",
          [BYTES_32_ZERO], // No invitation code
          [5000n],
        ],
        { account: creator.account }
      );
      ...
      await expect(
        distributor.write.joinPool([1n, BYTES_32_ZERO], {
          account: addr2.account,
        })
      ).to.be.rejectedWith("InvalidInvitationCode");
    });
    ...
    it("Should handle multiple invitation slots correctly", async function () {
      ...
      const code1 = generateInvitation();
      const code2 = generateInvitation();
    ...
    it("Should work with donations and withdrawals after joining", async function () {
      const { distributor, usdc, creator, addr2, addr3 } = await loadFixture(
        deployDistributorFixture
      );

      // Get balances before
      const balanceCreatorBefore = await usdc.read.balanceOf([
        creator.account.address,
      ]);
      const balanceAddr3Before = await usdc.read.balanceOf([
      ...
      const balanceCreatorAfter = await usdc.read.balanceOf([
        creator.account.address, // owner is the creator
      ]);
      const balanceAddr3After = await usdc.read.balanceOf([
        addr3.account.address,
      ]);

      expect(balanceCreatorAfter - balanceCreatorBefore).to.equal(parseUnits("800", 6)); // 80% of 1000 (creator percentage)
      expect(balanceAddr3After - balanceAddr3Before).to.equal(parseUnits("200", 6)); // 20% of 1000
    });
  ...
  describe("View Methods", function () {
    ...
    describe("getPoolSummary", function () {
      it("Should return empty summary for new pool", async function () {
        ...
        const summary = await distributor.read.getPoolSummary([
          singleMemberPoolId,
        ]);
      ...
      it("Should track donations correctly", async function () {
        const { distributor, usdc, creator, addr2, addr3, singleMemberPoolId } =
          await loadFixture(deployDistributorFixtureWithPool);
        ...
        await usdc.write.approve([distributor.address, donationAmount1], {
          account: addr2.account,
        });
        ...
        await usdc.write.approve([distributor.address, donationAmount2], {
          account: addr3.account,
        });
        ...
        expect(summary.totalDonationsAmount).to.equal(
          donationAmount1 + donationAmount2 + donationAmount1
        );
    ...
    describe("getPoolMembersCount", function () {
      it("Should return 1 for single member pool", async function () {
        ...
        const count = await distributor.read.getPoolMembersCount([
          singleMemberPoolId,
        ]);
      ...
      it("Should return correct count for pool with invitation slots", async function () {
        ...
        const invitation1 = generateInvitation();
        const invitation2 = generateInvitation();
    ...
    describe("getPoolMembers", function () {
      it("Should return all members with default pagination", async function () {
        ...
        const members = await distributor.read.getPoolMembers([
          singleMemberPoolId,
          0n,
          100n,
        ]);
      ...
      it("Should handle pagination correctly", async function () {
        ...
        const invitation1 = generateInvitation();
        const invitation2 = generateInvitation();
        const invitation3 = generateInvitation();
        ...
        expect(firstPage[0].member).to.equal(
          getAddress(creator.account.address)
        );
        expect(firstPage[1].member).to.equal(
          "0x0000000000000000000000000000000000000000"
        );
        ...
        expect(secondPage[0].member).to.equal(
          "0x0000000000000000000000000000000000000000"
        );
        expect(secondPage[1].member).to.equal(
          "0x0000000000000000000000000000000000000000"
        );

        // Test beyond bounds
        const beyondBounds = await distributor.read.getPoolMembers([
          1n,
          10n,
          5n,
        ]);
      ...
      it("Should show invitation code hashes for empty slots", async function () {
        ...
        expect(members[0].invitationCodeHash).to.equal(BYTES_32_ZERO);
        // Invitation slot
        expect(members[1].member).to.equal(
          "0x0000000000000000000000000000000000000000"
        );
    ...
    describe("getUserSummary", function () {
      it("Should return empty summary for user with no pools", async function () {
        ...
        const summary = await distributor.read.getUserSummary([
          addr3.account.address,
        ]);
      ...
      it("Should aggregate data from multiple pools", async function () {
        const { distributor, usdc, creator } = await loadFixture(
          deployDistributorFixture
        );

        // Create first pool
        await distributor.write.createPool(
          ["Pool 1", "First pool", "https://example.com/image1.png", [], []],
          { account: creator.account }
        );

        // Create second pool
        await distributor.write.createPool(
          ["Pool 2", "Second pool", "https://example.com/image2.png", [], []],
          { account: creator.account }
        );

        // Donate to first pool
        const donation1 = parseUnits("500", 6);
        await usdc.write.approve([distributor.address, donation1], {
          account: creator.account,
        });
        ...
        await usdc.write.approve([distributor.address, donation2 + donation3], {
          account: creator.account,
        });
      ...
      it("Should include pools where user joined via invitation", async function () {
        ...
        const addr2Summary = await distributor.read.getUserSummary([
        ...
        const creatorSummary = await distributor.read.getUserSummary([
          creator.account.address,
        ]);
    ...
    describe("getUserPoolsCount", function () {
      it("Should return 0 for user with no pools", async function () {
        ...
        const count = await distributor.read.getUserPoolsCount([
          addr3.account.address,
        ]);
        expect(count).to.equal(0n);
      });
      ...
      it("Should increase count when user creates new pools", async function () {
        ...
        const initialCount = await distributor.read.getUserPoolsCount([
          creator.account.address,
        ]);

        // Create another pool
        await distributor.write.createPool(
          ["New Pool", "Another pool", "https://example.com/image.png", [], []],
          { account: creator.account }
        );

        const newCount = await distributor.read.getUserPoolsCount([
          creator.account.address,
        ]);
    ...
    describe("getUserPools", function () {
      it("Should return empty array for user with no pools", async function () {
        ...
        const pools = await distributor.read.getUserPools([
          addr3.account.address,
          0n,
          100n,
        ]);
      ...
      it("Should handle pagination correctly", async function () {
        ...
        const firstPage = await distributor.read.getUserPools([
          creator.account.address,
          0n,
          2n,
        ]);
        ...
        // Test second page (offset 2, limit 2)
        const secondPage = await distributor.read.getUserPools([
          creator.account.address,
          2n,
          2n,
        ]);
        ...
        // Test beyond bounds
        const beyondBounds = await distributor.read.getUserPools([
          creator.account.address,
          10n,
          5n,
        ]);
